...
static LRESULT CALLBACK WindowProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	switch(msg) {
	case WM_CLOSE:
		if (MessageBox(hwnd, "Are you sure?", "Close", MB_OKCANCEL) == IDOK)
		{
			DestroyWindow(hwnd);
		}
	...
	case WM_SIZE:
		...
		if (mouseState != NULL) {
			mouseInfo.state = STATE_SIZING;
			mouseState->emit("WIN_SIZING", mouseInfo);
		}
		break;

	case WM_PAINT:
		std::cout << "need paint" << std::endl;
		//HandlePaint(pState, ...);
		if (mouseState != NULL) {
			//MouseStPoint mouseInfo;
			mouseInfo.state = STATE_PAINTING;
			mouseState->emit("WIN_PAINTING", mouseInfo);
		}
		break;
	  ...
	  case WM_LBUTTONDOWN:
        if(wipInput != NULL) {
			...
			if (mouseState != NULL) {
				//MouseStPoint mouseInfo;
				mouseInfo.state = STATE_DOWN;
				mouseInfo.mouseDown = true;
				mouseInfo.x = int(LOWORD(lParam)); // error prone area
				mouseInfo.y = int(HIWORD(lParam)); // error prone area
				mouseState->emit("MOUSE_DOWN",mouseInfo);
			}
		}
		SetCapture(hwnd);
 		return 0;
	  ...
	  case WM_LBUTTONUP:
        if(wipInput != NULL) {
			...
			if (mouseState != NULL) {
				//MouseStPoint mouseInfo;
				mouseInfo.state = STATE_UP;
				mouseInfo.mouseDown = false;
				mouseInfo.x = int(LOWORD(lParam)); // error prone area
				mouseInfo.y = int(HIWORD(lParam)); // error prone area
				mouseState->emit("MOUSE_UP", mouseInfo);
			}
		}
		ReleaseCapture();
 		return 0;
	  ...
	  case WM_MOUSEMOVE:
        ...
        if(wipInput != NULL) {
			...
			if (mouseState != NULL) {
				//MouseStPoint mouseInfo;
				mouseInfo.state = STATE_MOVE;
				mouseInfo.x = int(LOWORD(lParam)); // error prone area
				mouseInfo.y = int(HIWORD(lParam)); // error prone area
				mouseState->emit("MOUSE_MOVE", mouseInfo);
			}
		}
 		return 0;
	  ...
	  case WM_KEYDOWN:
	    switch(wParam) {
		  case VK_DELETE:
			  //std::cout << "Delete Key Pressed" << endl;
			  if (mouseState != NULL) {
				  mouseInfo.delKey = 1;
				  mouseState->emit("DELETE_KEY", mouseInfo);
			  }
			  break;
		  case VK_CONTROL:
			  if (mouseState != NULL) {
...
hInstance(GetModuleHandle(0)), iWindowWidth(iWindWidth), iWindowHeight(iWindHeight) {
	...
	hwndWindow = CreateWindow(ccDefWindClassName, ccDefWindTitle, WS_OVERLAPPEDWINDOW, iWindXPos,
